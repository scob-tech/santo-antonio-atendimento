// icons.js
// Ícones da interface (Lucide — https://lucide.dev, licença ISC). Ficam
// embutidos aqui como texto SVG pra não depender de CDN nem de build:
// qualquer tela chama icone('nome') e recebe o <svg> pronto. Elementos
// estáticos do HTML usam <span data-icon="nome"></span> e são preenchidos
// uma vez no carregamento por hidratarIcones().
const ICONES = {
...
function icone(nome, tamanho = 18, classe = '') {
  const corpo = ICONES[nome];
  if (!corpo) return '';
  return `<svg class="ic ${classe}" width="${tamanho}" height="${tamanho}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${corpo}</svg>`;
}

function hidratarIcones(raiz = document) {
  raiz.querySelectorAll('[data-icon]').forEach((el) => {
    if (el.dataset.iconOk) return;
    el.innerHTML = icone(el.dataset.icon, Number(el.dataset.size) || 18);
    el.dataset.iconOk = '1';
  });
}
hidratarIcones();
